<template>
  <div>
          ...
          <div class="kt-portlet__head-toolbar">
            <div class="kt-portlet__head-wrapper">
              <div class="kt-portlet__head-actions">
                <a data-toggle="modal" href="#newItem" class="btn btn-success btn-elevate btn-icon-sm">
                  <i class="la la-plus"></i>
                  Добавить предмет
                </a>
                <a style="margin-left: 10px;" href="#" @click="refreshPrices" class="btn btn-success btn-elevate btn-icon-sm">
    ...
    <div class="modal fade" id="newItem" tabindex="-1" role="dialog" aria-labelledby="newLabel" style="display: none;" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLongTitle">Новый предмет</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
          </div>
          <form class="kt-form-new" onclick="return false;">
            <div class="modal-body">
              <div class="form-group">
                <label>Ссылка:</label>
                <input placeholder="https://steamcommunity.com/market/listings/730/M4A4%20%7C%20Desolate%20Space%20%28Field-Tested%29" type="text" class="form-control" v-model="newItem.url">
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button>
              <button type="submit" class="btn btn-primary" v-on:click="createItem">Добавить</button>
            </div>
          </form>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async loadTable() {
      ...
      table.DataTable({
        ...
        columns: [
          ...
          { data: "icon_url", searchable: true,
            render: function (data, type, row) {
              return `<img src="https://community.cloudflare.steamstatic.com/economy/image/${row.icon_url}/62x62" />`
            }
          },
    ...
    async createItem() {
      $.wnoty({
        type: 'warning',
        message: 'Ищем предмет'
      })

      this.$root.request('POST', `/admin/items/create`, this.newItem)
          .then(() => {
            $('#newItem').modal('hide')
